...
def load_data(output_size):
    ...
    folds = folds_file['folds_ep_icd9_multi'][0][0]

    data_file = np.load(data_filename, allow_pickle=True)
    y = data_file['y_icd9'][:, :output_size]
    y = (y > 0).astype(float)
...
def run_experiment(queue, experiment, model, X, y, train_idx, test_idx, metric_list, output_size):
    ...
    for i in range(output_size):
        ...
        single_output_results['output'] = i + 1
...
def main():
    ...
    X, y, folds = load_data(output_size)
    ...
    results_filename = os.path.join(results_folder, 'experiment_results_icd9.csv')

    fieldnames = list(configurations[0].keys())
    fieldnames += [metric.name for metric in metric_list]
    fieldnames += ['collecting_datasets', 'training', 'training_per_epoch', 'prediction', 'output']
    ...
    for i, experiment_config in progress_bar:
        for j, (train_idx, valid_idx, test_idx) in enumerate(folds):
            ...
            model = build_model(model_config, n_features=X.shape[1], output_size=output_size)
            ...
            p = Process(
                ...
                args=(queue, experiment, model, X, y, train_idx, test_idx, metric_list, output_size)
            )
            p.start()
            p.join()
            results = queue.get()
